...
void UABCharacterMovementComponent::SetTepelortCommand()
{
...
void UABCharacterMovementComponent::ABTeleport()
{
	if (CharacterOwner)
	{
		AB_SUBLOG(LogAB, Log, TEXT("%s"), TEXT("Begin"));
		...
		GetWorld()->GetTimerManager().SetTimer(Handle, FTimerDelegate::CreateLambda([&]
			{
				bDidTeleport = false;
				AB_SUBLOG(LogAB, Log, TEXT("%s"), TEXT("End")); 
			}
		), TeleportCooltime, false, -1.0);
	}
}
